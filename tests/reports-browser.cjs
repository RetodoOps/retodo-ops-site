...
(async()=>{
 ...
 await page.locator('#statusSummary').click();await page.locator('#statusOptions input').first().check();await page.locator('#statusOptions input').nth(1).check();await page.locator('#applyReport').click();await page.getByText('1–50 of 55',{exact:true}).waitFor();assert.deepEqual(lastRequest.p_filters.statuses,['Ongoing','Approved']);
 ...
 await page.locator('#reportTable').scrollIntoViewIfNeeded();assert.ok((await page.locator('#reportTable').boundingBox()).height>100);await page.locator('.reports-main').evaluate(e=>e.scrollTop=0);await page.screenshot({path:'/tmp/update059-projects-desktop.png'});
 ...
 role='qa';await page.goto(base+'reports.html?type=margin');await page.getByText('1–50 of 55',{exact:true}).waitFor();assert.equal(await page.locator('#exportReport').isEnabled(),true);assert.equal(await page.locator('#currency').isEnabled(),true);
 await page.locator('.nav-item').filter({hasText:'Clients'}).click();await page.locator('#sub-cli').waitFor({state:'visible'});
 await page.setViewportSize({width:390,height:844});await page.screenshot({path:'/tmp/update059-margin-mobile.png'});
