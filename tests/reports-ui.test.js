...
test('CSV retains report metadata, raw IDs, currency buckets and warnings',()=>{
 ...
 assert.ok(csv.includes('"id-1","p-1"'));assert.ok(csv.includes("\"'=1\""));assert.ok(csv.includes('""USD"":4'));assert.ok(csv.includes('Currency conversion required'));assert.ok(csv.includes('Project date'));
});
