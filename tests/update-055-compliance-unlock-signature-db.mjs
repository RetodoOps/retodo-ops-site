...
await db.exec(read('tms/migrations/051_update_055_compliance_unlock_retodo_signature.sql'));

await check('unlocking Compliance atomically signs Agreement 1.1 for Retodo', async () => {
  const providerProfileId = await insertProfile('resource');
  const newResourceId = await insertResource('RO-LNG-00555', providerProfileId, {
    legalName: 'Update 055 Vendor', email: 'vendor055@example.com',
    taxId: 'VAT-055', phase: 'Not requested',
  });
  await db.query(`
    INSERT INTO resource_tests(
      resource_id,test_type,status,test_result,assigned_at,completed_at,created_by
    ) VALUES($1,'General','Completed','Pass',NOW(),NOW(),$2)
  `, [newResourceId, adminId]);

  await claim('service_role'); await actor(adminId);
  const unlocked = (await db.query(`
    SELECT public.resource_compliance_workflow_dispatch_055('request',$1,$2,NULL) AS value
  `, [adminId, newResourceId])).rows[0].value;
  assert.equal(unlocked.status, 'Requested');
  assert.equal(unlocked.agreement_version, '1.1');
  assert.equal(unlocked.retodo_signed, true);

  const issuance = (await db.query(`
    SELECT * FROM resource_framework_agreement_issuances WHERE resource_id=$1
  `, [newResourceId])).rows[0];
  assert.equal(issuance.retodo_signatory_name, 'Demir Atanasov');
  assert.equal(issuance.retodo_signatory_title, 'Owner');
  assert.equal(issuance.signed_by, adminId);
  assert.equal(issuance.agreement_sha256,
    '9886350a460367dcd0b2f64a76e4e1535bb9f5d7a3930f60c9545fbcb487294c');

  await claim('authenticated'); await actor(providerProfileId); await ownResource(newResourceId);
  const offered = (await db.query(
    'SELECT public.resource_portal_framework_agreement_050() AS value',
  )).rows[0].value;
  assert.equal(offered.status, 'Awaiting Service Provider signature');
  assert.equal(offered.can_accept, true);
  assert.equal(offered.retodo.signatory_name, 'Demir Atanasov');
  assert.ok(offered.retodo.signed_at);

  const signed = (await db.query(`
    SELECT public.resource_portal_accept_framework_agreement_050($1) AS value
  `, [JSON.stringify({
    service_provider_name: 'Update 055 Vendor', registration_or_id_number: 'VAT-055',
    service_provider_address: 'Sofia, Bulgaria', tax_vat_number: 'VAT-055',
    signatory_name: 'Update 055 Signer', accepted: true,
  })])).rows[0].value;
  assert.equal(signed.status, 'Signed');
  assert.equal(signed.agreement_version, '1.1');
  assert.equal(signed.provider.signatory_name, 'Update 055 Signer');
  assert.ok(signed.accepted_at);
  assert.ok(signed.effective_date);

  const stored = (await db.query(`
    SELECT retodo_signatory_name,retodo_signed_by,retodo_signed_at,accepted_by,accepted_at
    FROM resource_framework_agreements WHERE resource_id=$1 AND agreement_version='1.1'
  `, [newResourceId])).rows[0];
  assert.equal(stored.retodo_signatory_name, 'Demir Atanasov');
  assert.equal(stored.retodo_signed_by, adminId);
  assert.equal(stored.accepted_by, providerProfileId);
  assert.ok(stored.retodo_signed_at);
  assert.ok(stored.accepted_at);
});

await check('audit 013 reports PASS for every Update 055 boundary', async () => {
  const result = await db.query(read('tms/audits/013_update_055_compliance_unlock_retodo_signature_audit.sql'));
  assert.ok(result.rows.length >= 6);
  for (const row of result.rows) assert.equal(row.result, 'PASS', row.check_name);
});

console.log(`${passed} Update 050-055 database checks passed`);
await db.close();
