...
await db.exec(`
INSERT INTO clients VALUES('${id(1)}','Example client');INSERT INTO client_accounts VALUES('${id(2)}','Localization');
INSERT INTO resources VALUES('${id(3)}','Resource one',null,'R1');
INSERT INTO projects VALUES('${id(10)}','P1','Project one','2026-09-24',null,'Ongoing','EUR','Alex','${id(1)}','${id(2)}');
...
await check('No-Scoop Project keeps unavailable value rather than synthetic zero',async()=>{await owner(`INSERT INTO projects SELECT '${id(11)}','P2','Empty','2026-09-24',null,'Assign','EUR','Alex','${id(1)}','${id(2)}'`);const row=(await report('projects',{search:'Empty'})).rows[0];assert.equal(row.client_value,null);assert.equal(row.profit,null);await owner(`DELETE FROM projects WHERE id='${id(11)}'`);});
