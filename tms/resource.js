...
let complianceSummary = null, complianceWorkflow = null, frameworkAgreement = null, complianceFileRecords = [], complianceRefreshTimer = null;
...
function jobQuantityLabel(quantity,unit){if(quantity==null||quantity==='')return 'Quantity not recorded';const number=Number(quantity).toLocaleString('en-GB',{maximumFractionDigits:3});return `${number} ${['Flat rate','Fixed fee'].includes(unit)?'flat rate':unit||'units'}`}
...
function renderComplianceWorkflow(){
    ...
    if(complianceWorkflow.submission_notification_error)notices.push('The Compliance submission is visible here, but its internal email alert was not delivered.');
    ...
    frameworkAgreement=complianceWorkflow.framework_agreement||null;
    renderFrameworkAgreementSummary();
}
...
function renderFrameworkAgreementSummary(){
    ...
    el('frameworkAgreementDetails').innerHTML=accepted
        ? `<div><span>Service Provider</span><strong>${esc(data.service_provider_name||'—')}</strong></div><div><span>ID / registration</span><strong>${esc(data.registration_or_id_number||'—')}</strong></div><div><span>Address</span><strong>${esc(data.service_provider_address||'—')}</strong></div><div><span>Tax / VAT</span><strong>${esc(data.tax_vat_number||'—')}</strong></div><div><span>Signatory</span><strong>${esc(data.signatory_name||'—')}</strong><small>${esc(data.registration_email||'—')}</small></div><div><span>Accepted</span><strong>${fmtDateTime(data.accepted_at)}</strong><small>Version ${esc(data.agreement_version||'1.0')}</small></div>`
...
async function deleteComplianceFile(fileId){
    if(!canEditCompliance())return showError('Operational access required.');
    const evidence=documents.find(row=>row.file_record_id===fileId),file=complianceFileById(fileId);
    if(!evidence||!file||!readyComplianceDocument(evidence))return showError('Compliance file unavailable.');
    const warning=evidence.status==='Valid'?' This evidence is already confirmed and its removal will affect the ISO calculation.':'';
    if(!confirm(`Permanently delete ${file.original_filename} from private storage?${warning} This cannot be undone.`))return;
    try{
        await complianceFileApi('delete_file',{file_id:fileId,reason:'Duplicate or incorrectly uploaded Compliance evidence deleted internally'});
        await loadResource();setStatus('Compliance evidence deleted ✓');
    }catch(error){showError(error.message)}
}
...
function renderHistory(){el('historyTbody').innerHTML=history.length?history.map(row=>`<tr><td>${row.period_start?fmtDate(row.period_start):row.project_year||'—'}${row.period_end?` – ${fmtDate(row.period_end)}`:''}</td><td>${esc(row.account_display_label||'Confidential account')}</td><td>${esc(row.source_language||'—')} → ${esc(row.target_language||'—')}</td><td>${esc(row.service_type||'—')}</td><td>${esc(specName(row.specialization_id))}</td><td>${esc(jobQuantityLabel(row.quantity,row.unit))}</td><td>${esc(row.project_summary||'—')}</td><td><input type="checkbox" ${row.include_in_blind_cv?'checked':''} onchange="toggleHistoryCv('${row.id}',this.checked)"></td></tr>`).join(''):'<tr class="state-row"><td colspan="8">No approved Job history.</td></tr>'}
...
const BLIND_CV_COMPANY_LINES=['Retodo EOOD · UIC 208524462','48A Svetla St., 1360 Sofia, Bulgaria','ops@retodo-ops.com · retodo-ops.com'];
function blindCvHistoryLine(item){return `${item.year||''} · ${cvText(item.account,'Confidential account')} · ${cvText(item.source_language,'—')} → ${cvText(item.target_language,'—')} · ${[item.service,item.specialization].filter(Boolean).join(', ')} · Approx. size: ${jobQuantityLabel(item.quantity,item.unit)}`}
function renderBlindCvPreview(){const r=cvData.resource,lp=cvData.language_pairs||[],hist=cvData.project_history||[];el('blindCvPreview').innerHTML=`<div class="cv-brand"><img src="../Logo-440x140.png" alt="Retodo Ops"><div><strong>Blind Linguist Profile</strong><span>${BLIND_CV_COMPANY_LINES.map(esc).join('<br>')}</span></div></div><h2>${esc(r.internal_number)} · ${esc(cvText(r.initials))}</h2><div class="cv-meta"><span><b>Nationality:</b> ${esc(cvText(r.nationality))}</span><span><b>Residence:</b> ${esc(cvText(r.country_of_residence))}</span><span><b>Native language:</b> ${esc(cvText(r.native_language))}</span></div><h3>Language coverage</h3>${lp.length?`<ul class="cv-language-list">${lp.map(x=>`<li>${esc(x.source)} → ${esc(x.target)}${x.native_target?' (native target)':''}</li>`).join('')}</ul>`:'<p>Not recorded</p>'}<h3>Professional experience</h3><ul>${blindCvExperienceLines().map(line=>`<li>${esc(line)}</li>`).join('')}</ul><h3>Services and specializations</h3><p><b>Services:</b> ${esc((cvData.services||[]).join(', ')||'Not recorded')}</p><ul>${(cvData.specializations||[]).map(x=>`<li>${esc(x.name)}${x.experience_years?` — ${x.experience_years} years`:''}</li>`).join('')||'<li>Not recorded</li>'}</ul><h3>Education</h3><ul>${(cvData.education||[]).map(x=>`<li>${esc([x.degree,x.field_of_study,x.institution].filter(Boolean).join(', '))}${x.end_year?` (${x.start_year||'—'}–${x.end_year})`:''}</li>`).join('')||'<li>Not recorded</li>'}</ul><h3>Selected project experience</h3><ul>${hist.map(x=>`<li>${esc(blindCvHistoryLine(x))}</li>`).join('')||'<li>No approved project history selected.</li>'}</ul><div class="cv-footer">Prepared by Retodo Ops · Personal identity and direct contact details withheld</div>`}
...
function activateResourceTab(tabName){const tab=[...document.querySelectorAll('.record-tab')].find(item=>item.dataset.tab===tabName);if(!tab)return false;document.querySelectorAll('.record-tab').forEach(item=>item.classList.toggle('active',item===tab));document.querySelectorAll('.record-pane').forEach(pane=>pane.classList.toggle('active',pane.id===`pane-${tabName}`));return true}
document.querySelectorAll('.record-tab').forEach(tab=>tab.addEventListener('click',()=>{activateResourceTab(tab.dataset.tab);window.history.replaceState(null,'',`${location.pathname}${location.search}#${tab.dataset.tab}`)}));
window.addEventListener('hashchange',()=>activateResourceTab(location.hash.slice(1)));
...
(async()=>{const user=await requireAuth();if(!user)return;await Promise.all([TMS_REF.loadLanguages(_sb),TMS_REF.loadServices(_sb)]);TMS_REF.installDatalists();TMS_REF.populateServiceSelect('service-name');resourceId=new URLSearchParams(location.search).get('id');if(!resourceId){location.href='resources.html?type=external';return}const role=await _sb.rpc('current_app_role');appRole=role.data||'user';await loadResource();activateResourceTab(location.hash.slice(1));scheduleComplianceRefresh()})();
