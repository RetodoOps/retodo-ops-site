...
function eligibleResource(resource){return resource.candidate_state!=='Blocked'&&!resource.requires_override&&['Assignable','Proven','Preferred'].includes(resource.resource_status)}
...
function matchingRate(rate,resourceId){const languageMatch=(rateValue,jobValue)=>!rateValue||rateValue===jobValue||String(jobValue||'').startsWith(`${rateValue} (`);return rate.resource_id===resourceId&&!rate.base_rate_id&&rate.status==='Approved'&&rate.service_type===job.service_type&&rate.unit===job.unit&&languageMatch(rate.source_language,job.source_language)&&languageMatch(rate.target_language,job.target_language)&&(!rate.specialization_id||rate.specialization_id===job.specialization_id)}
...
async function searchCandidates(){
  ...
  wrap.innerHTML=candidateRows.map(resource=>{const blocked=resource.candidate_state==='Blocked',warning=resource.candidate_state==='Warning',css=blocked?'candidate-warning':warning?'candidate-warning':'',pill=blocked?'pill-red':warning?'pill-amber':'pill-green';return `<label class="candidate-row ${css}"><input type="radio" name="candidate" value="${resource.id}" ${blocked?'disabled':''}><span><strong>${esc(resource.internal_number)} · ${esc(resourceName(resource))}</strong><small>${esc((resource.source_languages||[]).join(', '))} → ${esc((resource.target_languages||[]).join(', '))} · ${esc((resource.services||[]).join(', '))}</small><small>${esc(resource.resource_status)} · Domain: ${esc(resource.specialization_qualification)} · Account: ${esc(resource.account_qualification)} · ${esc(resource.availability_status||'Unknown availability')}</small>${resource.warning_text?`<small class="warning-text">${esc(resource.warning_text)}</small>`:''}</span><span class="pill ${pill}">${esc(resource.candidate_state)}</span></label>`}).join('');
...
async function createOffer(){
  clearError('offerError');if(!selectedCandidateId)return showError('Select a Resource.','offerError');if(!val('o-rate'))return showError('Select an approved matching rate from the Resource profile.','offerError');const selectedCandidate=candidateRows.find(resource=>resource.id===selectedCandidateId);if(selectedCandidate?.requires_override&&!document.getElementById('o-override').checked)return showError('This Resource requires an Administrator override and a reason.','offerError');
