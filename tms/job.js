...
function rateMismatchReasons(rate,resourceId){const reasons=[],languageMatch=(rateValue,jobValue)=>!rateValue||rateValue===jobValue||String(jobValue||'').startsWith(`${rateValue} (`);if(rate.resource_id!==resourceId)reasons.push('different Resource');if(rate.base_rate_id)reasons.push('CAT child row');if(rate.status!=='Approved')reasons.push(`status ${rate.status}`);if(rate.active===false)reasons.push('inactive');if(rate.service_type!==job.service_type)reasons.push(`Service ${rate.service_type} ≠ ${job.service_type}`);if(rate.unit!==job.unit)reasons.push(`Unit ${rate.unit} ≠ ${job.unit||'not set'}`);if(!languageMatch(rate.source_language,job.source_language))reasons.push(`Source ${rate.source_language||'Any'} ≠ ${job.source_language||'not set'}`);if(!languageMatch(rate.target_language,job.target_language))reasons.push(`Target ${rate.target_language||'Any'} ≠ ${job.target_language||'not set'}`);if(rate.specialization_id&&rate.specialization_id!==job.specialization_id)reasons.push(`Specialization ${specName(rate.specialization_id)} ≠ ${specName(job.specialization_id)}`);return reasons}
