...
function jobFinancialSnapshot(){
  ...
  const supplierCost=supplierRows.length?supplierRows.reduce((sum,row)=>sum+roundMoney(row.unit==='Fixed fee'?(Number(row.quantity||0)>0?Number(row.unit_price||0):0):Number(row.quantity||0)*Number(row.unit_price||0)),0):(job?.resource_id?Number(job.supplier_amount||0):null);
  const quantityRows=supplierRows.length?supplierRows:[{cat_band:null,quantity:Number(val('j-quantity')||job?.quantity||0),unit:terms.unit}];
  let clientValue=0,matched=0;
  quantityRows.forEach(row=>{
    const matches=projectScopeLines.filter(line=>line.service_type===terms.service_type&&line.specialization_id===terms.specialization_id&&line.price_unit===terms.unit&&(row.cat_band?catBandKey(line.cat_band)===catBandKey(row.cat_band):!line.cat_band));
    matches.forEach(line=>{clientValue+=roundMoney(line.price_unit==='Fixed fee'?(Number(row.quantity||0)>0?Number(line.unit_price||0):0):Number(row.quantity||0)*Number(line.unit_price||0));matched++});
  });
  return {clientValue:matched?roundMoney(clientValue):null,clientCurrency:project?.currency||'EUR',supplierCost:supplierCost==null?null:roundMoney(supplierCost),supplierCurrency};
}
...
function renderOffers(){
  ...
  if(!offers.length){body.innerHTML='<tr class="state-row"><td colspan="7">No assignment history yet.</td></tr>';return}
...
function renderPO(){
  ...
  document.getElementById('poVersions').innerHTML=poVersions.length?poVersions.map(versionRow=>{const email=poEmails.find(record=>record.purchase_order_id===purchaseOrder.id&&Number(record.po_version||1)===Number(versionRow.version_number));const emailState=email?.status==='Sent'?'Email sent':email?.status==='Failed'?'Email failed':'Not sent';return `<div class="version-row"><strong>Version ${versionRow.version_number}</strong><span>${esc(versionRow.document_status)} · ${emailState} · ${new Date(versionRow.created_at).toLocaleString('en-GB')}</span><span>${esc(versionRow.change_reason||'Initial issue')}</span></div>`}).join(''):'<span class="muted">No issued versions yet.</span>';
  calculatePOTotals();
}

function renderPOHistory(){const card=document.getElementById('poHistoryCard'),list=document.getElementById('poHistory');card.classList.toggle('hidden',!purchaseOrders.length);if(!purchaseOrders.length){list.innerHTML='';return}list.innerHTML=purchaseOrders.map(po=>`<div class="version-row"><strong>${esc(po.po_number)}</strong><span><span class="pill ${po.status==='Cancelled'?'pill-red':po.status==='Issued'?'pill-green':''}">${esc(po.status)}</span> · Version ${po.current_version}</span><span>${esc(po.last_change_reason||'Initial assignment')}</span></div>`).join('')}
...
async function loadJob(){
  ...
  if(purchaseOrder){const [lineResult,versionResult]=await Promise.all([_sb.from('supplier_po_lines').select('*').eq('purchase_order_id',purchaseOrder.id).order('sort_order'),_sb.from('supplier_po_versions').select('*').eq('purchase_order_id',purchaseOrder.id).order('version_number',{ascending:false})]);poVersions=versionResult.data||[];const latestVersion=poVersions[0],snapshotLines=latestVersion?.snapshot?.lines;poLines=Array.isArray(snapshotLines)?snapshotLines:(lineResult.data||[]);purchaseOrder.current_version=Math.max(Number(purchaseOrder.current_version||0),...poVersions.map(version=>Number(version.version_number||0)))}else{poLines=[];poVersions=[]}
...
document.getElementById('j-rate-select').addEventListener('change',()=>renderSupplierCatGrid('j-rate-select'));
['j-service','j-source','j-target','j-specialization','j-unit'].forEach(id=>document.getElementById(id).addEventListener('change',()=>{const resourceId=val('j-candidate')||job.resource_id;if(resourceId)fillRateSelect('j-rate-select',resourceId)}));
document.getElementById('j-quantity').addEventListener('input',renderJobFinancials);
