...
const displayRate = amount => {const value=Number(amount||0),rounded=Math.round((value+Number.EPSILON)*100)/100;return Math.abs(value-rounded)<0.00011?rounded.toFixed(2).replace(/\.00$/,''):value.toFixed(4).replace(/0+$/,'').replace(/\.$/,'')};
const unitMoney = (amount, currency = purchaseOrder?.currency || job?.supplier_currency || 'EUR') => `${displayRate(amount)} ${currency}`;
...
function inheritedSpecializationId(service=val('j-service')){return projectScopeLines.find(line=>line.project_scoop_id===job?.project_scoop_id&&line.service_type===service&&line.specialization_id)?.specialization_id||projectSpecializations[0]?.specialization_id||job?.specialization_id||''}
function syncInheritedSpecialization(){const id=inheritedSpecializationId();document.getElementById('j-specialization').value=id;document.getElementById('j-specialization-display').textContent=id?specName(id):'Project specialization is missing'}
...
function inheritedProjectCatQuantities(){const terms=overviewTerms(),result=new Map();projectScopeLines.filter(line=>line.project_scoop_id===job?.project_scoop_id&&line.service_type===terms.service_type&&line.specialization_id===terms.specialization_id&&line.price_unit===terms.unit&&line.cat_band).forEach(line=>result.set(catBandKey(line.cat_band),Number(line.quantity||0)));return result}
function displayedJobCatRows(){return [...document.querySelectorAll('#j-cat-rows tr')].map(row=>({cat_band:row.dataset.band||null,quantity:Number(row.querySelector('.supplier-cat-quantity')?.value||0),unit:row.dataset.unit,unit_price:Number(row.dataset.rate||0),currency:row.dataset.currency||job?.supplier_currency||'EUR'}))}
...
function currentJobClientValue(terms,supplierRows){
  if(scoop?.price!=null)return roundMoney(scoop.price);
...
function currentPOVersion(){return purchaseOrder?Math.max(expectedPOVersion,Number(purchaseOrder.current_version||0),...poVersions.map(version=>Number(version.version_number||0))):0}
...
function renderPOEmail(){
  ...
  const version=currentPOVersion(),email=currentPOEmail(),recipient=purchaseOrder.supplier_snapshot?.email||assignedResource?.email||'Resource email',canSend=['admin','pm','client_relations'].includes(currentRole);status.classList.remove('hidden');button.classList.remove('hidden');button.disabled=!canSend;button.title=canSend?'':'Operational role required';
...
function renderPO(){
  ...
  const draft=purchaseOrder.status==='Draft',admin=currentRole==='admin',operations=['admin','pm','client_relations'].includes(currentRole),canRevise=['admin','pm'].includes(currentRole),canEdit=draft?operations:canRevise;document.getElementById('savePoBtn').textContent=draft?'Save Draft':'Create Revision';document.getElementById('savePoBtn').disabled=!canEdit;document.getElementById('savePoBtn').title=canEdit?'':draft?'Operational role required':'Administrator or Project Manager role required';document.getElementById('issuePoBtn').classList.toggle('hidden',!draft);document.getElementById('issuePoBtn').disabled=!admin;document.getElementById('revisionReasonWrap').classList.toggle('hidden',draft);
  ...
  document.querySelectorAll('#poLinesTbody input,#poLinesTbody select,.add-line-btn').forEach(el=>el.disabled=!canEdit);
  calculatePOTotals();
}
...
async function savePO(){const lines=collectPOLines();if(!lines.length)return showError('At least one PO line is required.');const wasDraft=purchaseOrder.status==='Draft';let result;if(wasDraft)result=await _sb.rpc('save_supplier_po_draft',{p_po_id:purchaseOrder.id,p_lines:lines});else{const reason=val('poRevisionReason');if(!reason)return showError('A revision reason is required.');result=await _sb.rpc('revise_supplier_po',{p_po_id:purchaseOrder.id,p_lines:lines,p_reason:reason})}if(result.error)return showError(result.error.message);if(!wasDraft)expectedPOVersion=Math.max(expectedPOVersion,Number(result.data||0));await loadJob();setStatus(wasDraft?'Draft PO saved ✓':`Supplier PO version ${Number(result.data||currentPOVersion())} created ✓`)}
...
(async()=>{currentUser=await requireAuth();if(!currentUser)return;await Promise.all([TMS_REF.loadLanguages(_sb),TMS_REF.loadServices(_sb)]);TMS_REF.installDatalists();TMS_REF.populateServiceSelect('j-service');currentRole=await currentAppRole()||'user';jobId=new URLSearchParams(location.search).get('id');if(!jobId){location.href='dashboard.html';return}await loadJob();const reportParams=new URLSearchParams(location.search),reportPo=reportParams.get('po'),reportVersion=Number(reportParams.get('version'));if(reportPo&&Number.isInteger(reportVersion)&&allPOVersions.some(row=>row.purchase_order_id===reportPo&&row.version_number===reportVersion))openPOVersion(reportPo,reportVersion)})();
